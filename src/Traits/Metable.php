...
use Traversable;
use Zdrojowa\AuthenticationLink\Models\Meta;
...
trait Metable
{
     ...
     * @return void
     */
    public function scopeOrderByMeta(Builder $q, string $key, string $direction = 'asc', $strict = false): void
    {
     ...
     * @return void
     */
    public function scopeOrderByMetaNumeric(Builder $q, string $key, string $direction = 'asc', $strict = false): void
    {
        $table = $this->joinMetaTable($q, $key, $strict ? 'inner' : 'left');
    ...
    private function joinMetaTable(Builder $q, string $key, $type = 'left'): string
    {
        ...
        $q->join("{$metaTable} as {$alias}", function(JoinClause $q) use ($relation, $key, $alias) {
            $q->on($relation->getQualifiedParentKeyName(), '=', $alias . '.' . $relation->getForeignKeyName())->where($alias . '.key', '=', $key)->where($alias . '.' . $relation->getMorphType(), '=', get_class($this));
        }, null, null, $type);
